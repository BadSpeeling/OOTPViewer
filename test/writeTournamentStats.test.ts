...
test('Populate BattingStats table with record', async () => {
    ...
    const battingRecord1 = await db.get(`select * from main.BattingStats where PtCardID = 1 and StatsBatchID = ${statsBatchID}`)
    const battingRecord2 = await db.get(`select * from main.BattingStats where PtCardID = 2319 and StatsBatchID = ${statsBatchID}`)
...
test('Populate PitchingStats table with pitching records', async () => {
  ...
  const pitchingRecord1 = await db.get(`select * from main.PitchingStats where PtCardID = 483 and StatsBatchID = ${statsBatchID}`)
  const pitchingRecord2 = await db.get(`select * from main.PitchingStats where PtCardID = 327 and StatsBatchID = ${statsBatchID}`)
